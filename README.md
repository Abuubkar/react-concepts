...
8.  **JavaScript Execution Contexts (two separate things to learn)**
    *   *The shared idea*: One tab is not one JavaScript world. Several contexts coexist, each with its own global object and its own heap, sharing no variables — they can only talk across a serialization boundary. This is the mental model underneath items 6 and 7: "the worker can't see my variable" and "there's no DOM in here" stop being surprises and become the obvious consequence of a separate global. Chrome exposes these through **two different pieces of DevTools UI**, because they are two different mechanisms. Learn both.

    **8a. Isolated worlds — the Console context dropdown**
    *   *What it is*: Same thread, same DOM, **separate JS context**. The dropdown in the Console toolbar reads `top` by default and lists the other contexts in the page: each **iframe**, and each **extension content script** (shown by extension name). "Isolated world" is Chrome's actual term for these — Blink calls them *worlds* internally, `main world` vs `isolated world`, so "isolated environment" is the right intuition with one word swapped.
    *   *Why it exists*: Security. A content script can read and modify the page's DOM, but its JavaScript is walled off — the page cannot reach into the extension's variables, and the extension can't accidentally collide with the page's. Without that boundary, any page could tamper with any extension running on it.
    *   *Try it*: In `top`, run `window.__marker = 'page'`. Switch the dropdown to a content-script context and evaluate `window.__marker` → `undefined`. Now run `document.title` in both → identical. Same DOM, different world. That single experiment is the whole concept. (The dropdown only lists extra entries when other contexts exist, so use a page with iframes and an active extension.)
    *   *Then go deeper*: MV3 lets a content script opt out with `"world": "MAIN"` in the manifest, running it in the page's own context instead — reading why you'd want that, and why it's a security downgrade, locks the distinction in. Also note cross-origin iframes: separate context *and*, under Site Isolation, a separate OS process, so `postMessage` is the only channel.

    **8b. Worker targets — Sources → Threads / Application → Service Workers**
    *   *What it is*: Separate **thread**, separate global (`self`, `DedicatedWorkerGlobalScope` / `ServiceWorkerGlobalScope`), **no DOM at all**. These are a different DevTools concept — a separate *target*, not an entry in the Console dropdown — which is why they live in different UI.
    *   *Where to find it*: **Sources → Threads** lists the page's threads; selecting one re-points the Console at that worker so you can evaluate inside it. For service workers, **Application → Service Workers → inspect** opens a dedicated DevTools window. `chrome://inspect` also lists workers globally.
    *   *Try it*: With a worker running, switch threads and evaluate `self`, `typeof window` (`undefined`), and `typeof document` (`undefined`). Set a breakpoint inside the worker and confirm the main thread keeps rendering while it's paused — that's the off-main-thread property from item 6, made visible.
    *   *Then go deeper*: Confirm the boundary is a real copy — `postMessage` an object, mutate it immediately afterwards, and watch the receiver hold the old value. That's structured clone, and it's exactly what `Transferable` and `SharedArrayBuffer` exist to avoid in items 6 and 7.

    *   *Everyday payoff*: React DevTools is itself a content script in an isolated world, which is why it needs a `postMessage` bridge to talk to the page's React internals rather than just reading them. Once 8a clicks, that architecture stops looking like overengineering.
